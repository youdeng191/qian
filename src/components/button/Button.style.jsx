...
${({ variant }) =>
    ...
    css`
      ...
      min-width: 270px;
      border-radius: 30px;
      background: #17181a; /* 背景色改为 #17181a */
      border: 2px solid ${({ theme }) => theme.colors.white}; /* 添加2px的白色边框 */
      box-shadow: 0 4px 6px rgba(0, 0, 0, 0.2); /* 添加边框阴影 */
      backdrop-filter: blur(10px);
      font-weight: 700;
      font-size: 16px;
      line-height: 26px;
      color: ${({ theme }) => theme.colors.white}; /* 文字颜色改为白色 */
      transition: 0.3s;

      &:hover {
        background: #17181a; /* 悬停时背景保持不变 */
        border: 2px solid ${({ theme }) => theme.colors.white}; /* 悬停时边框保持不变 */
        box-shadow: 0 4px 8px rgba(0, 0, 0, 0.4); /* 悬停时阴影加深 */
        color: ${({ theme }) => theme.colors.white}; /* 悬停时文字保持白色 */
      }
    `}
